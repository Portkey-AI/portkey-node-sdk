...
import { TranscriptionSessionCreateParams } from 'openai/resources/beta/realtime/transcription-sessions';
...
export class Realtime extends ApiResource {
  ...
  constructor(client: any) {
    ...
    this.transcriptionSessions = new TranscriptionSessions(client);
  }
}
...
export class TranscriptionSessions extends ApiResource {
  async create(
    _body: TranscriptionSessionCreateParams,
    params?: ApiClientInterface,
    opts?: RequestOptions
  ): Promise<any> {
    const body: TranscriptionSessionCreateParams = _body;
    if (params) {
      const config = overrideConfig(this.client.config, params.config);
      this.client.customHeaders = {
        ...this.client.customHeaders,
    ...
    const OAIclient = initOpenAIClient(this.client);
    const response = await OAIclient.beta.realtime.transcriptionSessions
      .create(body, opts)
      .withResponse();
